...
class StableDiffusionPipeline:
    ...
    def __init__(
        ...
        def makeScheduler(cls, subfolder="scheduler", **kwargs):
            return cls.from_pretrained(get_path(self.version, self.pipeline_type), subfolder=subfolder, use_karras_sigmas=True)
        ...
        if scheduler == "DDIM":
            ...
            self.scheduler = makeScheduler(DPMSolverMultistepScheduler)
    ...
    def run(self, prompt, negative_prompt, height, width, batch_size, batch_count, num_warmup_runs, use_cuda_graph, seed, **kwargs):
        # Process prompt
        ...
        if num_warmup_runs > 0:
            ...
            for _ in range(num_warmup_runs):
                self.infer(prompt, negative_prompt, height, width, seed=seed, warmup=True, **kwargs)

        for _ in range(batch_count):
            print("[I] Running StableDiffusion pipeline")
            if self.nvtx_profile:
                cudart.cudaProfilerStart()
            self.infer(prompt, negative_prompt, height, width, seed=seed, warmup=False, **kwargs)
            if self.nvtx_profile:
